...
```typescript
import { typeOf } from '@runtyped/type';
...
The type object is a simple object literal that contains a `kind` property that indicates the type of the type object. The `kind` property is a number and gets its meaning from the enum `ReflectionKind`. `ReflectionKind` is defined in the `@runtyped/type` package as follows:
...
Note that the type of `Reflection.propertySignature` has changed to `Reflection.property` and `Reflection.methodSignature` has changed to `Reflection.method`. Since properties and methods on classes have additional attributes, this information can also be retrieved. The latter additionally include visibility, abstract, and default. Type objects of classes contain only the properties and methods of the class itself and not of super-classes. This is contrary to type objects of interfacesobject-literals, which have all property signatures and method signatures of all parents resolved into types. To resolve the property and methods of the super-classes, either `ReflectionClass` and its `ReflectionClass.getProperties()` (see following sections) or `resolveTypeMembers()` of `@runtyped/type` can be used.

There is a whole plethora of type objects. For example for literal, template literals, promise, enum, union, array, tuple, and many more. To find out which ones all exist and what information is available, it is recommended to import `Type` from `@runtyped/type`. It is a union with all possible subtypes like `TypeAny`, `TypeUnknonwn`, `TypeVoid`, `TypeString`, `TypeNumber`, `TypeObjectLiteral`, `TypeArray`, `TypeClass`, and many more. There you can find the exact structure.
...
```typescript
import { ReflectionClass } from '@runtyped/type';
...
```typescript
import { resolveReceiveType, ReceiveType } from '@runtyped/type';
