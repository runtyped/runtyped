...
export const validators: Record<string, SchemaMapper> = {
  ...
  decimal(
    s,
    minDigits: TypeLiteral & { literal: number },
    maxDigits: TypeLiteral & { literal: number },
  ): Schema {
    return {
      ...s,
      pattern:
        "^-?\\d+\\.\\d{" + minDigits.literal + "," + maxDigits.literal + "}$",
    };
  },
  multipleOf(
    s,
    num: TypeLiteral & { literal: number }
  ): Schema {
    if (num.literal === 0) throw new TypeNotSupportedError(num, `multiple cannot be 0`);
  ...
  minLength(
    s,
    length: TypeLiteral & { literal: number }
  ): Schema {
    if (length.literal < 0)  throw new TypeNotSupportedError(length, `length cannot be less than 0`);
  ...
  includes(s, include: TypeLiteral): Schema {
    throw new TypeNotSupportedError(include, `includes is not supported. `);
  },
  excludes(s, exclude: TypeLiteral): Schema {
    throw new TypeNotSupportedError(exclude, `excludes is not supported. `);
  },
