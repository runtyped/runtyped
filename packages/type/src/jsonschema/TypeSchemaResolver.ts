import {
  JsonSchemaErrors,
  LiteralNotSupportedError,
  TypeNotSupportedError,
} from "./errors.js";
import { AnySchema, Schema } from "./types.js";
import { getParentClass } from "@runtyped/core";
import {
  isDateType,
  ReflectionKind,
  Type,
...
import { reflect } from "../reflection/reflection.js";
...
export class TypeSchemaResolver {
  ...
  constructor(public t: Type, public schemaRegisty: SchemaRegistry) {}
    ...
    switch (this.t.kind) {
      ...
      case ReflectionKind.literal:
        ...
        if (type) {
          ...
          this.errors.push(new LiteralNotSupportedError(typeof this.t.literal));
        }
      ...
      case ReflectionKind.templateLiteral:
        ...
        this.errors.push(
          new TypeNotSupportedError(
            this.t,
            "Literal is treated as string for simplicity",
          ),
        );
      ...
      case ReflectionKind.union:
        ...
        return;
      default:
        this.errors.push(new TypeNotSupportedError(this.t));
        return;
    }
  }
    ...
    for (const typeClass of typeClasses) {
      for (const typeItem of typeClass!.types) {
        if (
          ...
          const typeResolver = resolveTypeSchema(
            typeItem.type,
            this.schemaRegisty,
          );
  ...
  resolveEnum() {
    ...
    for (const value of this.t.values) {
      ...
      if (currentType === undefined) {
        this.errors.push(
          new TypeNotSupportedError(this.t, `Enum with unsupported members. `),
        );
        continue;
      }
  ...
  resolveValidators() {
    for (const annotation of validationAnnotation.getAnnotations(this.t)) {
      ...
      if (!validator) {
        this.errors.push(
          new TypeNotSupportedError(this.t, `Validator ${name} is not supported. `),
        );
      } else {
        try {
          this.result = validator(this.result, ...(args as [any]));
        } catch (e) {
          if (e instanceof TypeNotSupportedError) {
            this.errors.push(e);
          } else {
            throw e;
          }
...
export const unwrapTypeSchema = (
  t: Type,
  r: SchemaRegistry = new SchemaRegistry(),
) => {
  ...
  if (resolver.errors.length === 0) {
    ...
    throw new JsonSchemaErrors(resolver.errors, "Errors with input type. ");
  }
};
