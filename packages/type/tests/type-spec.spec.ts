...
import { AutoIncrement, BackReference, MapName, MongoId, PrimaryKey, Reference, UUID, isReferenceType } from '../src/type-annotations.js';
...
test('relation 1', () => {
...

    {
        ...
        // Round trip: serialize (FK) -> deserialize (creates Reference)
        const roundTripped = roundTrip<Team>(team);
        expect(roundTripped.id).toBe(team.id);
        expect(roundTripped.lead!.id).toBe(12);
    }
